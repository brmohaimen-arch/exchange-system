...
export function TreasuryShell({ visibleTabs, pageTitle, basePath }: TreasuryShellProps) {
  return (
    <Suspense fallback={<div className="flex h-64 items-center justify-center text-muted-foreground text-sm">جاري التحميل...</div>}>
      <TreasuryShellInner visibleTabs={visibleTabs} pageTitle={pageTitle} basePath={basePath} />
    </Suspense>
  )
}
...
function TreasuryShellInner({ visibleTabs, pageTitle, basePath }: TreasuryShellProps) {
  ...
  const initialTab = (requestedTab && visibleTabs.includes(requestedTab) ? requestedTab : visibleTabs[0]) as TabKey
