...
class Customer(Base):
    ...
    bank_account_number: Mapped[str | None] = mapped_column(String(100), nullable=True)  # IBAN or account number, entered as given
...
class Advance(Base):
    """A cash advance (سلفة) handed to a customer straight out of a vault or bank
    account — unlike Debt, which is a pure paper record that never moves cash,
    creating an Advance immediately debits the source, and repaying one credits
    it back. Tracked as its own third number alongside a customer's balance and
    debt, not merged into either. Exactly one of vault_id/bank_account_id is set,
    mirroring the same choice on a customer deposit/withdrawal."""
    ...
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    remaining_amount: Mapped[float] = mapped_column(Float, nullable=False)
    vault_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("vaults.id"), nullable=True)
    vault_name: Mapped[str | None] = mapped_column(String(100), nullable=True)
    bank_account_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("bank_accounts.id"), nullable=True)
...
class BankDeposit(Base):
    ...
    deposit carries its own interest rate set when it was made (banks quote a rate per
    deposit, not one blanket annual rate for the whole account) — amount and rate are
    entered manually, accrued_interest is computed automatically from them."""
    __tablename__ = "bank_deposits"
    id: Mapped[str] = mapped_column(String(50), primary_key=True)
    bank_account_id: Mapped[str] = mapped_column(String(50), ForeignKey("bank_accounts.id"))
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    currency: Mapped[str] = mapped_column(String(10), ForeignKey("currencies.code"))
    interest_rate: Mapped[float] = mapped_column(Float, default=0.0)
    deposit_date: Mapped[str] = mapped_column(String(50), nullable=False)
    accrued_interest: Mapped[float] = mapped_column(Float, default=0.0)
    last_calculated: Mapped[str | None] = mapped_column(String(50), nullable=True)
...
class FixedAsset(Base):
    ...
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Only meaningful when type == "مخزن" (warehouse) and that warehouse record
    # itself represents one specific car being stored — a separate, lighter-weight
    # path than creating a full Vehicle record for it.
    ...
    make_year: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class Vehicle(Base):
    ...
    warehouse_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("fixed_assets.id"), nullable=True)  # the fixed_assets row (type="مخزن") this car is stored at, if any
    barcode: Mapped[str | None] = mapped_column(String(100), nullable=True)  # entered manually by the operator; camera-scanned only to look the vehicle back up, never generated by the system
...
class CustomerDocument(Base):
    ...
    id: Mapped[str] = mapped_column(String(50), primary_key=True)
    customer_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("customers.id"), nullable=True)  # null until connected to an actual customer record
    customer_name: Mapped[str | None] = mapped_column(String(150), nullable=True)
...
class CustomerAccountEntry(Base):
    ...
    customer_id: Mapped[str] = mapped_column(String(50), ForeignKey("customers.id"))
    customer_name: Mapped[str] = mapped_column(String(150), nullable=False)
    vault_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("vaults.id"), nullable=True)  # set when funded from a vault's cash drawer
    vault_name: Mapped[str | None] = mapped_column(String(100), nullable=True)
    bank_account_id: Mapped[str | None] = mapped_column(String(50), ForeignKey("bank_accounts.id"), nullable=True)  # set when funded from a bank account instead
    bank_account_name: Mapped[str | None] = mapped_column(String(150), nullable=True)
    other_source: Mapped[str | None] = mapped_column(String(200), nullable=True)  # free-text label when funded from neither a vault nor a bank account
    currency: Mapped[str] = mapped_column(String(10), ForeignKey("currencies.code"))
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    balance_before: Mapped[float] = mapped_column(Float, nullable=False)
    balance_after: Mapped[float] = mapped_column(Float, nullable=False)
